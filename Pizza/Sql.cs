...
internal static class Sql
{
    ...
    public static DbType<User> GetUserByID(int id)
    {
        ...
        using (MySqlConnection con = new(conStr))
        {
            ...
            using (MySqlDataReader reader = cmd.ExecuteReader())
            {
                if (!reader.Read())
                {
                    return new DbType<User>(new User(reader));
                }
            }
        }

        return new DbType<User>(true);
    }
